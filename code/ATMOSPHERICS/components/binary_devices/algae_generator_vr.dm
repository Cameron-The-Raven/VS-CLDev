...
/obj/machinery/atmospherics/binary/algae_farm
	...
	var/const/input_gas = REAGENT_ID_CARBON_DIOXIDE
	var/const/output_gas = REAGENT_ID_OXYGEN
...
/obj/machinery/atmospherics/binary/algae_farm/process()
	...
	if(co2_moles < MINIMUM_MOLES_TO_FILTER)
		ui_error = "Insufficient [SSchemistry.get_gas_name(input_gas)] to process."
		update_icon()
		return
...
/obj/machinery/atmospherics/binary/algae_farm/tgui_data(mob/user)
	...
	if(air1 && network1 && node1)
		data["input"] = list(
			...
			"name" = SSchemistry.get_gas_name(input_gas),
	...
	if(air2 && network2 && node2)
		data["output"] = list(
			...
			"name" = SSchemistry.get_gas_name(output_gas),
