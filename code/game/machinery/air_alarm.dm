...
/obj/machinery/alarm/server/Initialize(mapload)
	...
	TLV[REAGENT_ID_OXYGEN] =			list(-1.0, -1.0,-1.0,-1.0) // Partial pressure, kpa
	TLV[REAGENT_ID_CARBON_DIOXIDE] = list(-1.0, -1.0,   5,  10) // Partial pressure, kpa
	TLV[REAGENT_ID_PHORON] =			list(-1.0, -1.0, 0, 0.5) // Partial pressure, kpa
...
/obj/machinery/alarm/proc/first_run()
	...
	TLV[REAGENT_ID_OXYGEN] =			list(16, 19, 135, 140) // Partial pressure, kpa
	TLV[REAGENT_ID_NITROGEN] =		list(0, 0, 135, 140) // Partial pressure, kpa
	TLV[REAGENT_ID_CARBON_DIOXIDE] = list(-1.0, -1.0, 5, 10) // Partial pressure, kpa
	TLV[REAGENT_ID_PHORON] =			list(-1.0, -1.0, 0, 0.5) // Partial pressure, kpa
	TLV["other"] =			list(-1.0, -1.0, 0.5, 1.0) // Partial pressure, kpa
...
/obj/machinery/alarm/proc/overall_danger_level(var/datum/gas_mixture/environment)
	...
	LOAD_TLV_VALUES(TLV[REAGENT_ID_OXYGEN], environment.gas[REAGENT_ID_OXYGEN]*partial_pressure)
	var/oxygen_dangerlevel = TEST_TLV_VALUES
	LOAD_TLV_VALUES(TLV[REAGENT_ID_CARBON_DIOXIDE], environment.gas[REAGENT_ID_CARBON_DIOXIDE]*partial_pressure)
	var/co2_dangerlevel = TEST_TLV_VALUES
	LOAD_TLV_VALUES(TLV[REAGENT_ID_PHORON], environment.gas[REAGENT_ID_PHORON]*partial_pressure)
...
/obj/machinery/alarm/tgui_data(mob/user, datum/tgui/ui, datum/tgui_state/state)
	...
	if(!locked || issilicon(user) || data["remoteUser"])
		...
		var/list/gas_names = list(REAGENT_ID_OXYGEN, REAGENT_ID_CARBON_DIOXIDE, REAGENT_ID_PHORON, "other")	//Gas ids made to match code\defines\gases.dm
