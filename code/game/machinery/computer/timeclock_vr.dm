...
/obj/machinery/computer/timeclock/tgui_data(mob/user)
	...
	if(card)
		...
		var/datum/job/job = SSoccupations.GetJob(card.rank)
		if(job)
			data["job_datum"] = list(
				"title" = job.title,
...
/obj/machinery/computer/timeclock/proc/makeOnDuty(var/newrank, var/newassignment, var/mob/user)
	var/datum/job/oldjob = SSoccupations.GetJob(card.rank)
	var/datum/job/newjob = SSoccupations.GetJob(newrank)
...
/obj/machinery/computer/timeclock/proc/makeOffDuty(var/mob/user)
	var/datum/job/foundjob = SSoccupations.GetJob(card.rank)
	if(!foundjob)
		return
