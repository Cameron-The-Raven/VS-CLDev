...
/obj/item/flamethrower/Topic(href,href_list[])
	...
	if(href_list["light"])
		if(!ptank)	return
		if(ptank.air_contents.gas[REAGENT_ID_PHORON] < 1)	return
		if(!status)	return
		lit = !lit
		if(lit)
...
/obj/item/flamethrower/proc/ignite_turf(turf/target)
	...
	new/obj/effect/decal/cleanable/liquid_fuel/flamethrower_fuel(target,air_transfer.gas[REAGENT_ID_PHORON],get_dir(loc,target))
	air_transfer.gas[REAGENT_ID_PHORON] = 0
