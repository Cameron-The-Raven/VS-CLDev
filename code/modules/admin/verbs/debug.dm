	...
	for(var/obj/machinery/power/rad_collector/Rad in machines)
		if(Rad.anchored)
			if(!Rad.P)
				...
				Phoron.air_contents.gas[REAGENT_ID_PHORON] = 70
				Rad.drainratio = 0
				Rad.P = Phoron
				Phoron.loc = Rad
...
/client/proc/setup_supermatter_engine()
	...
	for(var/obj/machinery/M in machines)
		...
		if(istype(M.loc.loc,/area/engineering/engine_room))
			if(istype(M,/obj/machinery/power/rad_collector))
				...
				Phoron.air_contents.gas[REAGENT_ID_PHORON] = 29.1154	//This is a full tank if you filled it from a canister
				Rad.P = Phoron
				...
				if(Pump.name == "Engine Feed" && response == "Setup Completely")
					found_the_pump = 1
					Pump.air2.gas[REAGENT_ID_NITROGEN] = 3750	//The contents of 2 canisters.
	...
	if(!found_the_pump && response == "Setup Completely")
		...
		T.zone.air.gas[REAGENT_ID_NITROGEN] += 450
