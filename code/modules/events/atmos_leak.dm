...
/datum/event/atmos_leak/setup()
	var/gas_choices = list(REAGENT_ID_CARBON_DIOXIDE, REAGENT_ID_NITROUS_OXIDE) // Annoying
	if(severity >= EVENT_LEVEL_MODERATE)
		gas_choices += REAGENT_ID_PHORON // Dangerous
	// if(severity >= EVENT_LEVEL_MAJOR)
	// 	gas_choices += REAGENT_ID_VOLATILE_FUEL // Dangerous and no default atmos setup!
...
/datum/event/atmos_leak/announce()
	command_announcement.Announce("Warning, hazardous [SSchemistry.get_gas_name(gas_type)] gas leak detected in \the [target_area], evacuate the area and contain the damage!", "Hazard Alert")
