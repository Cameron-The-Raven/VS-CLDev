...
/datum/supply_demand_order/gas/describe()
	...
	for(var/gas in mixture.gas)
		desc += "<br>- [SSchemistry.get_gas_name(gas)]: [round((mixture.gas[gas] / total_moles) * 100)]%\n"
	return desc
...
/datum/event/supply_demand/proc/choose_atmos_items(var/differentTypes)
	...
	var/unpickedTypes = SSchemistry.atmo_gases.Copy()
	unpickedTypes -= REAGENT_ID_VOLATILE_FUEL // Don't do that one
	for(var/i in 1 to differentTypes)
