...
*/
	...
	if(exude_gasses && exude_gasses.len)
		for(var/gas in exude_gasses)
			...
			data["trait_info"] += "It will release [amount][SSchemistry.get_gas_name(gas)] into the environment."
	...
	if(consume_gasses && consume_gasses.len)
		for(var/gas in consume_gasses)
			...
			if (consume_gasses[gas] > 7)
				...
				amount = "small amounts of "
			data["trait_info"] += "It will consume [amount][SSchemistry.get_gas_name(gas)] from the environment."
