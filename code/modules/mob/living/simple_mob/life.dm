...
/mob/living/simple_mob/proc/handle_atmos()
	...
	if(istype(A,/turf))
		...
		if(Environment)
			...
			if(min_oxy && Environment.gas[REAGENT_ID_OXYGEN] < min_oxy)
				atmos_unsuitable = 1
				throw_alert("oxy", /obj/screen/alert/not_enough_oxy)
			else if(max_oxy && Environment.gas[REAGENT_ID_OXYGEN] > max_oxy)
				atmos_unsuitable = 1
				throw_alert("oxy", /obj/screen/alert/too_much_oxy)
			else
				clear_alert("oxy")

			if(min_tox && Environment.gas[REAGENT_ID_PHORON] < min_tox)
				atmos_unsuitable = 2
				throw_alert("tox_in_air", /obj/screen/alert/not_enough_tox)
			else if(max_tox && Environment.gas[REAGENT_ID_PHORON] > max_tox)
				atmos_unsuitable = 2
				throw_alert("tox_in_air", /obj/screen/alert/tox_in_air)
			else
				clear_alert("tox_in_air")

			if(min_n2 && Environment.gas[REAGENT_ID_NITROGEN] < min_n2)
				atmos_unsuitable = 1
				throw_alert("n2o", /obj/screen/alert/not_enough_nitro)
			else if(max_n2 && Environment.gas[REAGENT_ID_NITROGEN] > max_n2)
				atmos_unsuitable = 1
				throw_alert("n2o", /obj/screen/alert/too_much_nitro)
			else
				clear_alert("n2o")

			if(min_co2 && Environment.gas[REAGENT_ID_CARBON_DIOXIDE] < min_co2)
				atmos_unsuitable = 1
				throw_alert("co2", /obj/screen/alert/not_enough_co2)
			else if(max_co2 && Environment.gas[REAGENT_ID_CARBON_DIOXIDE] > max_co2)
				atmos_unsuitable = 1
