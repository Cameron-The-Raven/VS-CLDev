...
/obj/machinery/power/rad_collector/process()
	...
	if(P)
		if(P.air_contents.gas[REAGENT_ID_PHORON] == 0)
			investigate_log("<font color='red'>out of fuel</font>.","singulo")
			eject()
		else
			P.air_contents.adjust_gas(REAGENT_ID_PHORON, -0.001*drainratio)
	return
...
/obj/machinery/power/rad_collector/attack_hand(mob/user as mob)
	if(anchored)
		if(!src.locked)
			...
			investigate_log("turned [active?"<font color='green'>on</font>":"<font color='red'>off</font>"] by [user.key]. [P?"Fuel: [round(P.air_contents.gas[REAGENT_ID_PHORON]/0.29)]%":"<font color='red'>It is empty</font>"].","singulo")
			return
		else
...
/obj/machinery/power/rad_collector/proc/receive_pulse(var/pulse_strength)
	if(P && active)
		...
		power_produced = P.air_contents.gas[REAGENT_ID_PHORON]*pulse_strength*20
