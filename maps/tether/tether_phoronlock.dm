...
/obj/machinery/airlock_sensor/phoron/process()
	if(on)
		...
		var/phoron = (REAGENT_ID_PHORON in air_sample.gas) ? round(air_sample.gas[REAGENT_ID_PHORON], 0.1) : 0
		...
		if(abs(pressure - previousPressure) > 0.1 || previousPressure == null || abs(phoron - previousPhoron) > 0.1 || previousPhoron == null)
			...
			signal.data[REAGENT_ID_PHORON] = num2text(phoron)
...
/datum/embedded_program/airlock/phoron/receive_signal(datum/signal/signal, receive_method, receive_param)
	...
	if(receive_tag==tag_chamber_sensor)
		memory["chamber_sensor_phoron"] = text2num(signal.data[REAGENT_ID_PHORON])
		memory["chamber_sensor_pressure"] = text2num(signal.data["pressure"])

	else if(receive_tag==tag_exterior_sensor)
		memory["external_sensor_phoron"] = text2num(signal.data[REAGENT_ID_PHORON])

	else if(receive_tag==tag_interior_sensor)
		memory["internal_sensor_phoron"] = text2num(signal.data[REAGENT_ID_PHORON])
